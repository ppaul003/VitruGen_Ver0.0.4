...
Decision: **CODE COMPLETE — INTERACTIVE ACCEPTANCE PENDING**
...
| 1E — Gate verification and freeze | Pending | Pending | Requires interactive runtime regression and user acceptance before tagging |
...
- `WorkspaceId` contains the canonical workspace identities, including
  `SINGLE_PARTICLE_MCAD`, `LINK_PARTICLES_MCAD`, and
  `PARTICLE_SIMULATION`.
...
The following tests must be performed on the Gate 1 branch before the gate is
tagged:

- [ ] Application starts and the main grid/interface renders normally.
- [ ] Top-level entry and back-navigation behave as they did at Gate 0.
- [ ] `SINGLE_PARTICLE_MCAD` configuration opens normally.
- [ ] The Single Particle sub-layer cycle remains intact.
- [ ] Guarded `Q` exit works from Single Particle sub-layer 0.
- [ ] Marching Cubes mesh generation completes.
- [ ] OBJ export completes and the resulting model is visually plausible.
- [ ] Particle simulation starts and animates normally.
- [ ] Re-entering both existing workflows does not crash or reset
  unexpectedly.
- [ ] No new CUDA, rendering, console, or UI anomaly is observed.

## Gate decision rule

When every interactive item passes:

1. Update this document to `Decision: PASS` and record the observed results.
2. Commit the accepted Gate 1 evidence.
3. Merge or fast-forward the accepted branch according to the repository
   workflow.
4. Create the annotated `gate-1` tag on the accepted commit.
5. Push the accepted branch/`main` and `gate-1` tag only after review.
