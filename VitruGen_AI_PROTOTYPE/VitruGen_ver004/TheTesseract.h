...
public:
	...
	enum WorkspaceBranch {
		...
		WORKSPACE_TEXTURE_MAP_2D,
		WORKSPACE_LINKED_PARTICLES_MCAD,
		WORKSPACE_SANDBOX_SIM,
		WORKSPACE_COUNT
	};
	...
	// --- SHARED MVP ASSET PIPELINE ---
	// TheTesseract owns authoring/runtime resources. TheArbiter only owns the
	// user-facing navigation and command state that selects these resources.
	vitru::ProjectAssetRepository& projectAssets() { return m_projectAssets; }
	const vitru::ProjectAssetRepository& projectAssets() const { return m_projectAssets; }
	vitru::SandboxRuntime& sandboxRuntime() { return m_sandboxRuntime; }
	const vitru::SandboxRuntime& sandboxRuntime() const { return m_sandboxRuntime; }

	vitru::AssetId captureMarchingCubesStaticParticle(
		const MarchingCubes& marchingCubes,
		const std::string& name
	);
	bool generateSurfaceMap(vitru::AssetId staticAssetId, std::uint32_t resolution = 512);
	vitru::AssetId createStarterAssembly(const std::string& name = "Untitled Machine");
	vitru::BakeResult bakeAssembly(vitru::AssetId assemblyId, const std::string& bakedName);
	bool spawnKinematicParticle(vitru::AssetId assetId, const vitru::Vec3& position = {});
	...
	void toggleLinkedAnimationPreview();
	bool isLinkedAnimationPreviewPlaying() const { return m_linkedPreviewPlaying; }
	float getLinkedAnimationPreviewTime() const { return m_linkedPreviewTimeSeconds; }

	vitru::AssetId getActiveStaticAssetId() const { return m_activeStaticAssetId; }
	vitru::AssetId getActiveAssemblyId() const { return m_activeAssemblyId; }
	vitru::AssetId getActiveKinematicAssetId() const { return m_activeKinematicAssetId; }
	void setActiveStaticAssetId(vitru::AssetId id) { m_activeStaticAssetId = id; }
	void setActiveAssemblyId(vitru::AssetId id) { m_activeAssemblyId = id; }
	void setActiveKinematicAssetId(vitru::AssetId id) { m_activeKinematicAssetId = id; }
...
private:
	...
	// Central project repository shared by every workspace.
	vitru::ProjectAssetRepository m_projectAssets;
	vitru::SandboxRuntime m_sandboxRuntime;
	vitru::AssetId m_activeStaticAssetId = vitru::INVALID_ASSET_ID;
	vitru::AssetId m_activeAssemblyId = vitru::INVALID_ASSET_ID;
	vitru::AssetId m_activeKinematicAssetId = vitru::INVALID_ASSET_ID;
	bool m_texturePainting = false;
	bool m_linkedPreviewPlaying = false;
	float m_linkedPreviewTimeSeconds = 0.0f;
