...
public:
	...
	void setParticleHighlighted(bool highligted) { m_particleHighlighted = highligted; }
	...
	void displayTextureMap2D(
		const vitru::StaticParticleAsset& asset,
		bool showUvOverlay,
		int viewportW,
		int viewportH
	);
	void displayLinkedAssembly(
	...
	void displayKinematicParticle(
		const vitru::KinematicParticleAsset& asset,
		const vitru::SandboxInstance* instance = nullptr,
		bool showNodes = true
	);
	void displaySandbox(const vitru::SandboxRuntime& sandbox);
	void displayStaticParticlePreview(
		const vitru::StaticParticleAsset& asset,
...
protected:
	...
	GLuint ensureMvpTexture(const vitru::StaticParticleAsset& asset);
	void drawMvpStaticMesh(const vitru::StaticParticleAsset& asset);
	void releaseMvpTextures();
	

private:
	static constexpr float kWorkspaceBoxSize = 4.0f;
	static constexpr float kWorkspaceHalfBox = kWorkspaceBoxSize * 0.5f;
	static constexpr int kWorkspaceGridDim = 16;
	static constexpr float kWorkspaceCellSize =
		kWorkspaceBoxSize / static_cast<float>(kWorkspaceGridDim);
	...
	WorkPlane m_workPlane = PLANE_XY;
	...
	struct MvpTextureResource {
		GLuint texture = 0;
		std::uint32_t width = 0;
		std::uint32_t height = 0;
		std::uint64_t contentHash = 0;
	};
	std::unordered_map<vitru::AssetId, MvpTextureResource> m_mvpTextures;
