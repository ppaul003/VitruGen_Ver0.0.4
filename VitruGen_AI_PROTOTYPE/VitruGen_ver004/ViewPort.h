...
public:
	...
	struct MvpPanelData {
		unsigned int staticAssetCount = 0;
		unsigned int assemblyCount = 0;
		unsigned int kinematicAssetCount = 0;
		unsigned int assemblyNodeCount = 0;
		unsigned int animationClipCount = 0;
		unsigned int sandboxHitCount = 0;
		unsigned int activeProjectileCount = 0;
		std::string activeStaticName;
		std::string activeAssemblyName;
		std::string activeKinematicName;
		std::string selectedNodeName;
		std::string selectedParentName;
		float selectedPosition[3]{};
		float selectedRotation[3]{};
		float selectedJointAxis[3]{};
		float selectedJointMinimum = 0.0f;
		float selectedJointMaximum = 0.0f;
		bool selectedNodeIsJoint = false;
		bool linkedPreviewPlaying = false;
		bool sandboxFollowCamera = false;
		std::string status;
	};
