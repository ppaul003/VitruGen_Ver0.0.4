...
float sdfDeltaWingApprox(
	...
	// Extruded isosceles triangle. The broad trailing edge is at -Y and the
	// point is at +Y, which makes it useful for wings, fins, and armor panels.
...
void mirroredVolumeKernel(
	...
	const uint w = static_cast<unsigned int>(volSize.x);
	const uint h = static_cast<unsigned int>(volSize.y);
	const uint index = c + r * w + s * w * h;

	const float localX =
		offset.x * mirrorBasisX.x +
		offset.y * mirrorBasisX.y +
		offset.z * mirrorBasisX.z;
	const float localY =
		offset.x * mirrorBasisY.x +
		offset.y * mirrorBasisY.y +
		offset.z * mirrorBasisY.z;
	const float localZ =
		offset.x * mirrorBasisZ.x +
		offset.y * mirrorBasisZ.y +
		offset.z * mirrorBasisZ.z;

	float distance = 1.0e20f;
	for (int variant = 0; variant < 8; ++variant) {
		if ((variant & ~mirrorAxisMask) != 0) continue;
		const float sx = (variant & 1) ? -1.0f : 1.0f;
		const float sy = (variant & 2) ? -1.0f : 1.0f;
		const float sz = (variant & 4) ? -1.0f : 1.0f;
		const float3 candidateOffset = make_float3(
			mirrorBasisX.x * localX * sx +
				mirrorBasisY.x * localY * sy +
				mirrorBasisZ.x * localZ * sz,
			mirrorBasisX.y * localX * sx +
				mirrorBasisY.y * localY * sy +
				mirrorBasisZ.y * localZ * sz,
			mirrorBasisX.z * localX * sx +
				mirrorBasisY.z * localY * sy +
				mirrorBasisZ.z * localZ * sz
		);
		const float candidateDistance = funcSDFOffset(
			static_cast<int>(c),
			static_cast<int>(r),
			static_cast<int>(s),
			id,
		...
		distance = fminf(distance, candidateDistance);
	}

	d_vol[index] = distance;
}
