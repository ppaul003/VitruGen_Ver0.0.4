...
namespace {
	...
	constexpr TheArbiter::WorkspaceId kMvpWorkspaceCycle[] = {
		TheArbiter::WorkspaceId::GRAPH_3D,
		TheArbiter::WorkspaceId::SINGLE_PARTICLE_MCAD,
		// Preserve the accepted Gate 1 D-key route before exposing the
		// connected authoring, assembly, and sandbox workspaces.
		TheArbiter::WorkspaceId::PARTICLE_SIMULATION,
		TheArbiter::WorkspaceId::TEXTURE_MAP_2D,
		TheArbiter::WorkspaceId::LINKED_PARTICLES_MCAD,
		TheArbiter::WorkspaceId::SANDBOX_SIM
	...
	constexpr int kMvpWorkspaceCycleCount =
		static_cast<int>(sizeof(kMvpWorkspaceCycle) /
			sizeof(kMvpWorkspaceCycle[0]));
...
void TheArbiter::toggleGridSelection() {
	cycleWorkspaceSelection(+1.0f);
}

void TheArbiter::cycleWorkspaceSelection(float dir) {
	int current = 0;
	for (int i = 0; i < kMvpWorkspaceCycleCount; ++i) {
		if (kMvpWorkspaceCycle[i] == getSelectedWorkspace()) {
			current = i;
			break;
		}
	}
	...
	if (dir < 0.0f)
		current = (current + kMvpWorkspaceCycleCount - 1) %
			kMvpWorkspaceCycleCount;
	else
		current = (current + 1) % kMvpWorkspaceCycleCount;

	const WorkspaceId workspace = kMvpWorkspaceCycle[current];
	setWorkspaceSelection(getWorkspaceDomain(workspace), workspace);
}
...
void TheArbiter::cycleVolumeMirrorMode(float dir) {
	int value = static_cast<int>(m_volumeMirrorMode);
	const int count = static_cast<int>(VOLUME_MIRROR_COUNT);
	...
	m_volumeMirrorMode = static_cast<VolumeMirrorMode>(value);
}

int TheArbiter::getVolumeMirrorAxisMask() const {
	switch (m_volumeMirrorMode) {
	case VOLUME_MIRROR_X: return 1;
	case VOLUME_MIRROR_Y: return 2;
	case VOLUME_MIRROR_Z: return 4;
	case VOLUME_MIRROR_XY: return 1 | 2;
	case VOLUME_MIRROR_XZ: return 1 | 4;
	case VOLUME_MIRROR_YZ: return 2 | 4;
	case VOLUME_MIRROR_XYZ: return 1 | 2 | 4;
	default:
	case VOLUME_MIRROR_NONE: return 0;
	}
}
...
void TheArbiter::resetAllVolumeStates() {
	...
	m_volumeMirrorMode = VOLUME_MIRROR_NONE;
}
...
TheArbiter::processKeyboard(const KeyboardInput::KeyEvent& event) {
	...
	case ApplicationLayer::ACTIVE_WORKSPACE:
		if (isTextureMapSelected()) {
			handleTextureWorkspaceKeyboard(event, result);
			break;
		}
		if (isLinkedParticlesSelected()) {
			handleLinkedWorkspaceKeyboard(event, result);
			break;
		}
		if (isSandboxSelected()) {
			handleSandboxWorkspaceKeyboard(event, result);
			break;
		}
...
const char* TheArbiter::getGridSelectionName() const {
	return getWorkspaceName(getSelectedWorkspace());
}
...
const char* TheArbiter::getSubLayerPanelListName() const {
	if (isMarchingCubesSubLayer()) {
		...
		case MC_LIST_SAVE_STATIC_ASSET: return "Save Static Particle";
		case MC_LIST_OPEN_TEXTURE: return "Open Texture Map";
...
const char* TheArbiter::getVolumeMirrorModeName() const {
	switch (m_volumeMirrorMode) {
	case VOLUME_MIRROR_X: return "X";
	case VOLUME_MIRROR_Y: return "Y";
	case VOLUME_MIRROR_Z: return "Z";
	case VOLUME_MIRROR_XY: return "XY";
	case VOLUME_MIRROR_XZ: return "XZ";
	case VOLUME_MIRROR_YZ: return "YZ";
	case VOLUME_MIRROR_XYZ: return "XYZ";
	default:
	case VOLUME_MIRROR_NONE: return "NONE";
	}
}

TheArbiter::TextureColor TheArbiter::getTexturePaintColor() const {
	switch (m_textureColorIndex) {
	case 1: return { 225, 55, 55, 255 };
	case 2: return { 65, 165, 90, 255 };
	case 3: return { 55, 120, 225, 255 };
	case 4: return { 235, 190, 55, 255 };
	case 5: return { 165, 80, 210, 255 };
	case 6: return { 25, 28, 32, 255 };
	case 7: return { 235, 238, 240, 255 };
	default:
	case 0: return { 125, 150, 170, 255 };
	}
}

const char* TheArbiter::getTexturePaintToolName() const {
	switch (m_texturePaintTool) {
	case TEXTURE_TOOL_ERASER: return "ERASER";
	case TEXTURE_TOOL_FILL: return "FILL";
	default:
	case TEXTURE_TOOL_BRUSH: return "BRUSH";
	}
}

const char* TheArbiter::getTextureColorName() const {
	switch (m_textureColorIndex) {
	case 1: return "RED";
	case 2: return "GREEN";
	case 3: return "BLUE";
...
const char* TheArbiter::getLinkedInteractionRoleName() const {
	switch (m_linkedInteractionRoleIndex) {
	case 1: return "WHEEL";
	case 2: return "LEFT PROPULSION";
	case 3: return "RIGHT PROPULSION";
	case 4: return "THRUSTER";
	case 5: return "WEAPON MUZZLE";
	case 6: return "SENSOR";
	case 7: return "SURFACE ANCHOR";
	default:
	case 0: return "CONTACT";
	}
}

void TheArbiter::handleTextureWorkspaceKeyboard(
	const KeyboardInput::KeyEvent& event,
	ArbiterResult& result) {
	auto moveCursor = [&](int direction) {
		m_activeTexturePanelItem =
			(m_activeTexturePanelItem + direction + TEXTURE_LIST_COUNT) %
			TEXTURE_LIST_COUNT;
	};
	...
	case KeyboardInput::KEY_A:
	case KeyboardInput::KEY_D: {
		const int direction = event.signal == KeyboardInput::KEY_A ? -1 : 1;
		if (m_activeTexturePanelItem == TEXTURE_LIST_COLOR) {
			constexpr int paletteSize = 8;
			m_textureColorIndex =
				(m_textureColorIndex + direction + paletteSize) % paletteSize;
		}
		else if (m_activeTexturePanelItem == TEXTURE_LIST_BRUSH_SIZE) {
			m_textureBrushSize = std::max(1,
				std::min(64, m_textureBrushSize + direction * 2));
		}
		else if (m_activeTexturePanelItem == TEXTURE_LIST_UV_OVERLAY) {
			m_textureUvOverlayVisible = !m_textureUvOverlayVisible;
		}
		break;
	}
	case KeyboardInput::KEY_E:
		switch (m_activeTexturePanelItem) {
		case TEXTURE_LIST_BRUSH:
			m_texturePaintTool = TEXTURE_TOOL_BRUSH;
			break;
		case TEXTURE_LIST_ERASER:
			m_texturePaintTool = TEXTURE_TOOL_ERASER;
			break;
		case TEXTURE_LIST_FILL:
			m_texturePaintTool = TEXTURE_TOOL_FILL;
			break;
		case TEXTURE_LIST_CLEAR:
			result.command = CMD_TEXTURE_CLEAR;
			break;
		case TEXTURE_LIST_IMPORT:
			result.command = CMD_TEXTURE_IMPORT;
			break;
		case TEXTURE_LIST_SAVE:
			result.command = CMD_TEXTURE_SAVE;
			break;
		case TEXTURE_LIST_UV_OVERLAY:
			m_textureUvOverlayVisible = !m_textureUvOverlayVisible;
			break;
		case TEXTURE_LIST_APPLY_RETURN:
			result.command = CMD_TEXTURE_APPLY;
			setWorkspaceSelection(
				WorkspaceDomain::GRID_3D,
				WorkspaceId::SINGLE_PARTICLE_MCAD
			);
	...
	result.requestRedraw = true;
	result.rebuildMenu = true;
	if (result.command == CMD_NONE) result.command = CMD_REDRAW;
}

void TheArbiter::handleLinkedWorkspaceKeyboard(
	const KeyboardInput::KeyEvent& event,
	ArbiterResult& result) {
	auto moveCursor = [&](int direction) {
		m_activeLinkedPanelItem =
			(m_activeLinkedPanelItem + direction + LINKED_LIST_COUNT) %
			LINKED_LIST_COUNT;
	};
	...
	case KeyboardInput::KEY_D: {
		const int direction = event.signal == KeyboardInput::KEY_A ? -1 : 1;
		if (m_activeLinkedPanelItem == LINKED_LIST_ADD_INTERACTION) {
			constexpr int roleCount = 8;
			m_linkedInteractionRoleIndex =
				(m_linkedInteractionRoleIndex + direction + roleCount) % roleCount;
		}
		else if (m_activeLinkedPanelItem >= LINKED_LIST_SELECT_NODE &&
			m_activeLinkedPanelItem <= LINKED_LIST_JOINT_LIMITS) {
			m_linkedAdjustmentDirection = direction;
			result.command = CMD_LINK_ADJUST;
		}
		break;
	}
	case KeyboardInput::KEY_E:
		switch (m_activeLinkedPanelItem) {
		case LINKED_LIST_CREATE_ASSEMBLY: result.command = CMD_LINK_CREATE_ASSEMBLY; break;
		case LINKED_LIST_ADD_MESH: result.command = CMD_LINK_ADD_MESH; break;
		case LINKED_LIST_ADD_FIXED_JOINT: result.command = CMD_LINK_ADD_FIXED_JOINT; break;
		case LINKED_LIST_ADD_REVOLUTE_JOINT: result.command = CMD_LINK_ADD_REVOLUTE_JOINT; break;
		case LINKED_LIST_ADD_INTERACTION: result.command = CMD_LINK_ADD_INTERACTION; break;
		case LINKED_LIST_ADD_ANIMATION: result.command = CMD_LINK_ADD_ANIMATION; break;
		case LINKED_LIST_BAKE: result.command = CMD_LINK_BAKE; break;
		case LINKED_LIST_SAVE_PROJECT: result.command = CMD_PROJECT_SAVE; break;
		case LINKED_LIST_OPEN_SANDBOX:
			result.command = CMD_OPEN_SANDBOX_SIM;
			setWorkspaceSelection(
				WorkspaceDomain::SIMCAD_4D,
				WorkspaceId::SANDBOX_SIM
			);
			break;
		case LINKED_LIST_LOAD_PROJECT: result.command = CMD_PROJECT_LOAD; break;
		case LINKED_LIST_PREVIEW_ANIMATION: result.command = CMD_LINK_PREVIEW_ANIMATION; break;
		case LINKED_LIST_REMOVE_NODE: result.command = CMD_LINK_REMOVE_NODE; break;
		default: break;
		}
		break;
	default:
...
void TheArbiter::handleSandboxWorkspaceKeyboard(
	const KeyboardInput::KeyEvent& event,
	ArbiterResult& result) {
	switch (event.signal) {
	case KeyboardInput::KEY_W: result.command = CMD_SANDBOX_DRIVE_FORWARD; break;
	case KeyboardInput::KEY_S: result.command = CMD_SANDBOX_DRIVE_REVERSE; break;
	case KeyboardInput::KEY_A: result.command = CMD_SANDBOX_TURN_LEFT; break;
	case KeyboardInput::KEY_D: result.command = CMD_SANDBOX_TURN_RIGHT; break;
	case KeyboardInput::KEY_E:
	case KeyboardInput::KEY_F: result.command = CMD_SANDBOX_FIRE; break;
	case KeyboardInput::KEY_R: result.command = CMD_SANDBOX_RESET; break;
	case KeyboardInput::KEY_SPACE: result.command = CMD_SANDBOX_PLAY_ANIMATION; break;
	case KeyboardInput::KEY_ENTER: result.command = CMD_SANDBOX_SPAWN; break;
	case KeyboardInput::KEY_Z: result.command = CMD_SANDBOX_JOINT_DECREASE; break;
	case KeyboardInput::KEY_X: result.command = CMD_SANDBOX_JOINT_INCREASE; break;
	case KeyboardInput::KEY_V: result.command = CMD_SANDBOX_TOGGLE_FOLLOW_CAMERA; break;
	default: result.command = CMD_REDRAW; break;
	}
	result.requestRedraw = true;
}
...
TheArbiter::activateMarchingCubesPanelItemFromMenu(MarchingCubesPanelItem item) {
	...
	const bool validItem =
		itemValue >=
		static_cast<int>(MC_LIST_SAVE_STATIC_ASSET) &&
		itemValue <
...
void TheArbiter::enterCurrentSelection(ArbiterResult& result) {
	...
	if (m_navigation.layer == ApplicationLayer::DOMAIN_SELECTION) {
		if (getWorkspaceAvailability(getSelectedWorkspace()) ==
			WorkspaceAvailability::AVAILABLE) {

			setApplicationLayer(ApplicationLayer::WORKSPACE_CONFIGURATION);
			m_activeParticleConfigList =
				(isTextureMapSelected() || isLinkedParticlesSelected() || isSandboxSelected())
				? PARTICLE_LIST_RUN
				: PARTICLE_LIST_COLOR;
		}
	...
	if (m_navigation.layer == ApplicationLayer::WORKSPACE_CONFIGURATION) {
		if (isTextureMapSelected() || isLinkedParticlesSelected() || isSandboxSelected()) {
			setApplicationLayer(ApplicationLayer::ACTIVE_WORKSPACE);
			m_activeTexturePanelItem = TEXTURE_LIST_BRUSH;
			m_activeLinkedPanelItem = LINKED_LIST_CREATE_ASSEMBLY;

			if (isTextureMapSelected())
				result.command = CMD_OPEN_TEXTURE_MAP_2D;
			else if (isLinkedParticlesSelected())
				result.command = CMD_OPEN_LINKED_PARTICLES_MCAD;
			else
				result.command = CMD_OPEN_SANDBOX_SIM;
...
void TheArbiter::activateSubLayerPanelItem(ArbiterResult& result) {
	...
	if (isMarchingCubesSubLayer()) {
		switch (m_activeSubLayerPanelItem) {
		case MC_LIST_SAVE_STATIC_ASSET:
			result.command = CMD_SAVE_STATIC_PARTICLE;
			break;

		case MC_LIST_OPEN_TEXTURE:
			result.command = CMD_OPEN_TEXTURE_MAP_2D;
			setWorkspaceSelection(
				WorkspaceDomain::GRID_2D,
	...
	case VOLUME_NODE_PREVIEW:
		...
		case PREVIEW_LIST_RUN_MC:
			m_singleParticleSubLayer = SP_SUB_LAYER_MARCHING_CUBES;
			m_activeSubLayerPanelItem = MC_LIST_SAVE_STATIC_ASSET;
			m_subLayerPanelOpen = true;
			result.enterMarchingCubes = true;
			result.rebuildMenu = true;
			break;
