...
void CameraProcessor::followWorldPoint(
	float x,
	float y,
	float z,
	float distance) {
	// The view transform applies yaw, then pitch, to the world point. Track
	// that rotated point so manual orbit remains available while following.
	const float radians = 0.01745329251994329577f;
	const float yaw = m_camera_rot[1] * radians;
	const float pitch = m_camera_rot[0] * radians;
	const float yawX = std::cos(yaw) * x + std::sin(yaw) * z;
	const float yawZ = -std::sin(yaw) * x + std::cos(yaw) * z;
	const float pitchY = std::cos(pitch) * y - std::sin(pitch) * yawZ;
	const float pitchZ = std::sin(pitch) * y + std::cos(pitch) * yawZ;
	m_camera_trans[0] = -yawX;
	m_camera_trans[1] = -pitchY - 0.65f;
	m_camera_trans[2] = -pitchZ - (distance < 1.5f ? 1.5f : distance);
}
