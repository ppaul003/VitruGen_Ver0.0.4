...
void ViewPort::drawOverlay(
	...
	if (arbiter.isMenuLayer()) {
		...
		drawLayer3SimulationRun(arbiter, mvpData, paused);
...
void ViewPort::drawSubLayerPanel(const TheArbiter& arbiter, const MarchingCubesPanelData* mcData) {
	...
	if (marchingCubesPanel) {
		...
		drawSubLayerPanelLine(
			labelX,
			y0 + 334.0f,
			activeItem == TheArbiter::MC_LIST_SAVE_STATIC_ASSET,
			"[1] Save Reusable Static Particle",
			alpha
		);
		...
		drawSubLayerPanelLine(
			labelX,
			y0 + 370.0f,
			activeItem == TheArbiter::MC_LIST_OPEN_TEXTURE,
			"[2] Generate Surface Map / Paint",
			alpha
		);
		...
		if (arbiter.hasInjectionVoxelSelected()) {
			...
			if (!editingVoxel1) {
				...
				snprintf(
					...
					sizeof(mirrorLine),
					"[4] Mirror { %s }",
					arbiter.getVolumeMirrorModeName()
				);
...
void ViewPort::drawLayer0Menu(const TheArbiter& arbiter) {
	...
	if (arbiter.isIdleSelected()) {
		...
		glColor4f(0.45f, 1.0f, 0.65f, m_panelSlide);
		drawText2D(panelX(95.0f), 310.0f, "WORKSPACE DOMAINS selected: press E to configure.", GLUT_BITMAP_HELVETICA_18);
	}
...
void ViewPort::drawLayer1EnvironmentConfig(const TheArbiter& arbiter) {
	...
	snprintf(
		line,
		sizeof(line),
		"[1]: WORKSPACE SELECTION { %s }",
		arbiter.getGridSelectionName()
	);

	drawSelectableLine(95.0f, 245.0f, true, line);

	if (TheArbiter::getWorkspaceAvailability(arbiter.getSelectedWorkspace()) !=
		TheArbiter::WorkspaceAvailability::AVAILABLE) {
		glColor4f(1.0f, 0.82f, 0.45f, m_panelSlide);
		drawText2D(panelX(95.0f), 310.0f, "This workspace is reserved or experimental.", GLUT_BITMAP_HELVETICA_18);
	}
	else {
		glColor4f(0.45f, 1.0f, 0.65f, m_panelSlide);
		drawText2D(panelX(95.0f), 310.0f, "AVAILABLE: press E to configure this workspace.", GLUT_BITMAP_HELVETICA_18);
	}
...
void ViewPort::drawLayer2ParticleConfig(const TheArbiter& arbiter, bool meshAvailable) {
	...
	if (arbiter.isTextureMapSelected() ||
		arbiter.isLinkedParticlesSelected() ||
		arbiter.isSandboxSelected()) {
		drawSelectableLine(
			95.0f,
			285.0f,
			true,
		...
		const char* pipelineText = arbiter.isTextureMapSelected()
			? "Paint the active static particle surface map."
			: (arbiter.isLinkedParticlesSelected()
				? "Build red meshes, blue joints, and green interactions."
				: "Spawn the last baked asset in the playable sandbox.");
		glColor4f(0.45f, 1.0f, 0.65f, m_panelSlide);
		drawText2D(panelX(95.0f), 350.0f, pipelineText, GLUT_BITMAP_HELVETICA_18);
		drawHelpFooter("E: Open workspace", "Q: Back one layer     ESC: Exit");
		drawWorkspaceFrame(0.20f, nullptr);
		return;
	}
...
void ViewPort::drawLayer3SimulationRun(
	...
	if (arbiter.isTextureMapSelected() ||
		arbiter.isLinkedParticlesSelected() ||
		arbiter.isSandboxSelected()) {
		const float panelRight = arbiter.isSandboxSelected() ? 690.0f :
			(arbiter.isLinkedParticlesSelected() ? 790.0f : 620.0f);
		const float panelBottom = static_cast<float>(m_window_h) - 24.0f;
		glColor4f(0.02f, 0.04f, 0.06f, 0.82f);
		glBegin(GL_QUADS);
		glVertex2f(24.0f, 24.0f);
		glVertex2f(panelRight, 24.0f);
		glVertex2f(panelRight, panelBottom);
		glVertex2f(24.0f, panelBottom);
		glEnd();

		glColor4f(0.82f, 0.95f, 1.0f, 0.92f);
		glBegin(GL_LINE_LOOP);
		glVertex2f(24.0f, 24.0f);
		glVertex2f(panelRight, 24.0f);
		glVertex2f(panelRight, panelBottom);
		...
		auto activeLine = [&](float y, bool active, const char* text) {
			if (active) {
				glColor4f(0.45f, 1.0f, 0.65f, 1.0f);
				drawText2D(48.0f, y, ">", GLUT_BITMAP_HELVETICA_18);
			}
			else glColor4f(0.72f, 0.78f, 0.82f, 1.0f);
			drawText2D(72.0f, y, text, GLUT_BITMAP_HELVETICA_18);
		};

		const char* status = mvpData && !mvpData->status.empty()
			? mvpData->status.c_str() : "MVP pipeline ready";
		glColor4f(0.45f, 1.0f, 0.65f, 1.0f);
		drawText2D(48.0f, 86.0f, status, GLUT_BITMAP_HELVETICA_12);

		if (arbiter.isTextureMapSelected()) {
			char toolLine[128];
			snprintf(toolLine, sizeof(toolLine), "[1] Paint Tool { %s }",
				arbiter.getTexturePaintToolName());
			char colorLine[128];
			snprintf(colorLine, sizeof(colorLine), "[2] Color { %s }",
				arbiter.getTextureColorName());
			char sizeLine[128];
			snprintf(sizeLine, sizeof(sizeLine), "[3] Brush Size { %d }",
				arbiter.getTextureBrushSize());
			char uvLine[128];
			snprintf(uvLine, sizeof(uvLine), "[9] UV Overlay { %s }",
				arbiter.isTextureUvOverlayVisible() ? "ON" : "OFF");
			const char* lines[] = {
				toolLine, colorLine, sizeLine, "[4] Select Eraser",
				"[5] Select Region Fill", "[6] Clear Texture",
				"[7] Import project import.ppm / import.bmp",
				"[8] Save Texture", uvLine,
				"[10] Apply Texture / Return to 3D"
			};
			for (int i = 0; i < TheArbiter::TEXTURE_LIST_COUNT; ++i) {
				activeLine(140.0f + i * 45.0f,
					arbiter.getActiveTexturePanelItem() == i, lines[i]);
			}
			glColor4f(0.75f, 0.78f, 0.82f, 1.0f);
			drawText2D(48.0f, panelBottom - 44.0f,
				"LMB/drag: Paint    W/S: Select    A/D: Adjust    E: Activate    Q: Back",
				GLUT_BITMAP_HELVETICA_12);
		}
		else if (arbiter.isLinkedParticlesSelected()) {
			char interactionLine[160];
			snprintf(interactionLine, sizeof(interactionLine),
				"[5] Add GREEN Interaction { %s }",
				arbiter.getLinkedInteractionRoleName());
			char selectedLine[256];
			snprintf(selectedLine, sizeof(selectedLine), "[10] Select Node { %s }",
				mvpData && !mvpData->selectedNodeName.empty()
				? mvpData->selectedNodeName.c_str() : "NONE");
			char parentLine[256];
			snprintf(parentLine, sizeof(parentLine), "[11] Select Parent { %s }",
				mvpData && !mvpData->selectedParentName.empty()
				? mvpData->selectedParentName.c_str() : "ROOT");
			char px[128], py[128], pz[128], rx[128], ry[128], rz[128];
			snprintf(px, sizeof(px), "[12] Local Position X { %.2f }", mvpData ? mvpData->selectedPosition[0] : 0.0f);
			snprintf(py, sizeof(py), "[13] Local Position Y { %.2f }", mvpData ? mvpData->selectedPosition[1] : 0.0f);
			snprintf(pz, sizeof(pz), "[14] Local Position Z { %.2f }", mvpData ? mvpData->selectedPosition[2] : 0.0f);
			snprintf(rx, sizeof(rx), "[15] Local Rotation X { %.1f }", mvpData ? mvpData->selectedRotation[0] : 0.0f);
			snprintf(ry, sizeof(ry), "[16] Local Rotation Y { %.1f }", mvpData ? mvpData->selectedRotation[1] : 0.0f);
			snprintf(rz, sizeof(rz), "[17] Local Rotation Z { %.1f }", mvpData ? mvpData->selectedRotation[2] : 0.0f);
			char axisLine[160];
			snprintf(axisLine, sizeof(axisLine), "[18] Joint Axis { %.0f, %.0f, %.0f }",
				mvpData ? mvpData->selectedJointAxis[0] : 0.0f,
				mvpData ? mvpData->selectedJointAxis[1] : 0.0f,
				mvpData ? mvpData->selectedJointAxis[2] : 0.0f);
			char limitsLine[160];
			snprintf(limitsLine, sizeof(limitsLine), "[19] Revolute Limits { %.0f .. %.0f }",
				mvpData ? mvpData->selectedJointMinimum : 0.0f,
				mvpData ? mvpData->selectedJointMaximum : 0.0f);
			const char* previewLine = mvpData && mvpData->linkedPreviewPlaying
				? "[21] Animation Preview { PLAYING }"
				: "[21] Animation Preview { STOPPED }";
			const char* lines[] = {
				"[1] Create Assembly From Saved Parts",
				"[2] Add RED Mesh Node",
				"[3] Add BLUE Fixed Joint",
				"[4] Add BLUE Revolute Joint",
				interactionLine,
				"[6] Define Named Joint Animation",
				"[7] Validate / Bake Kinematic Particle",
				"[8] Save Project",
				"[9] Open SANDBOX_SIM",
				selectedLine,
				parentLine,
				...
				axisLine,
				limitsLine,
				"[20] Reopen Saved Project",
				previewLine,
				"[22] Remove Selected Node Branch"
			};
			for (int i = 0; i < TheArbiter::LINKED_LIST_COUNT; ++i) {
				activeLine(125.0f + i * 30.0f,
					arbiter.getActiveLinkedPanelItem() == i, lines[i]);
			}
			if (mvpData) {
				char diagnostics[256];
				snprintf(diagnostics, sizeof(diagnostics),
					"Assets %u | Nodes %u | Clips %u | Baked %u",
					mvpData->staticAssetCount,
					mvpData->assemblyNodeCount,
					mvpData->animationClipCount,
					mvpData->kinematicAssetCount);
				glColor4f(0.65f, 0.88f, 1.0f, 1.0f);
				drawText2D(48.0f, panelBottom - 72.0f,
					diagnostics, GLUT_BITMAP_HELVETICA_12);
			}
			glColor4f(0.75f, 0.78f, 0.82f, 1.0f);
			drawText2D(48.0f, panelBottom - 44.0f,
				"W/S: Select    A/D: Edit value/selection    E: Apply action    Q: Back",
				GLUT_BITMAP_HELVETICA_12);
		}
		else {
			glColor4f(0.85f, 0.95f, 1.0f, 1.0f);
			drawText2D(48.0f, 140.0f,
				"RUNTIME DIAGNOSTICS", GLUT_BITMAP_HELVETICA_18);
			char assetLine[256];
			snprintf(assetLine, sizeof(assetLine), "Spawned: %s",
				mvpData && !mvpData->activeKinematicName.empty()
				? mvpData->activeKinematicName.c_str() : "NONE");
			drawText2D(48.0f, 190.0f, assetLine, GLUT_BITMAP_HELVETICA_18);
			char counts[256];
			snprintf(counts, sizeof(counts),
				"Targets Hit: %u    Active Projectiles: %u",
				mvpData ? mvpData->sandboxHitCount : 0,
				mvpData ? mvpData->activeProjectileCount : 0);
			drawText2D(48.0f, 235.0f, counts, GLUT_BITMAP_HELVETICA_18);
			drawText2D(48.0f, 305.0f,
				"W / S     DRIVE / REVERSE", GLUT_BITMAP_HELVETICA_18);
			drawText2D(48.0f, 345.0f,
				"A / D     TURN LEFT / RIGHT", GLUT_BITMAP_HELVETICA_18);
			drawText2D(48.0f, 385.0f,
				"E or F    FIRE PROJECTILE", GLUT_BITMAP_HELVETICA_18);
			drawText2D(48.0f, 425.0f,
				"SPACE     PLAY JOINT ANIMATION", GLUT_BITMAP_HELVETICA_18);
			drawText2D(48.0f, 465.0f,
				"ENTER     SPAWN / RESPAWN", GLUT_BITMAP_HELVETICA_18);
			drawText2D(48.0f, 505.0f,
				"Z / X     DIRECT REVOLUTE JOINT CONTROL", GLUT_BITMAP_HELVETICA_18);
			drawText2D(48.0f, 545.0f,
				mvpData && mvpData->sandboxFollowCamera
				? "V         FOLLOW CAMERA { ON }" : "V         FOLLOW CAMERA { OFF }",
				GLUT_BITMAP_HELVETICA_18);
			drawText2D(48.0f, 585.0f,
				"R         RESET SANDBOX", GLUT_BITMAP_HELVETICA_18);
			drawText2D(48.0f, 625.0f,
				"Q         RETURN TO CONFIGURATION", GLUT_BITMAP_HELVETICA_18);
		}
		return;
	}
