...
private:
	...
	void handleMvpCommand(TheArbiter::ArbiterCommand command);
	void setMvpStatus(const std::string& status);
...
private:
	...
	std::string m_mvpProjectPath =
		"VITRUGEN_PROJECT_DATA/VitruGen_MVP.vitru";
	std::string m_mvpStatus = "MVP pipeline ready";
	vitru::NodeId m_linkedSelectedNodeId = vitru::INVALID_NODE_ID;
	vitru::NodeId m_linkedParentNodeId = vitru::INVALID_NODE_ID;
	vitru::NodeId m_linkedLastJointNodeId = vitru::INVALID_NODE_ID;
	bool m_sandboxFollowCamera = false;
	int m_staticAssetNameCounter = 1;
